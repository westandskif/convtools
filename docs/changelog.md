...
- join condition terms that reference `c.LEFT`/`c.RIGHT` can no longer use the join input (`c.this`); they raise `ConversionException` at `gen_converter()`. Pass the value via `c.input_arg(...)` or a label, or attach the expression with `.pipe(...)`. Helper-based constructs used bare next to `c.LEFT`/`c.RIGHT` (`c.try_(...).except_(...)`, and `c.if_multiple`/`c.dispatch` whose subtree uses `c.this`) now raise the same way; rewrite as `c.LEFT.item(...).pipe(c.try_(...))` or hoist
