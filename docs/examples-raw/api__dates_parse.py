...
# SINGLE FORMAT
converter = c.date_parse("%m/%d/%Y").gen_converter()
assert converter("12/31/2020") == date(2020, 12, 31)

# MULTIPLE FORMATS
converter = (
    c.iter(c.date_parse("%m/%d/%Y", "%Y-%m-%d")).as_type(list).gen_converter()
)
...
converter = c.item("dt").datetime_parse("%m/%d/%Y %H:%M").gen_converter()
